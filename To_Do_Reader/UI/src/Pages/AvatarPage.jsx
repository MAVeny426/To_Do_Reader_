...
const AvatarPage = () => {
  ...
  useEffect(() => {
    const fetchUserProfileAndTasks = async () => {
      ...
      try {
        const userRes = await fetch('https://to-do-reader-server.onrender.com/api/auth/me', {
          method: 'GET',
          headers: {
        ...
        const completedTasksRes = await fetch('https://to-do-reader-server.onrender.com/api/tasks/mycompleted', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}` 
